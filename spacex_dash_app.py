...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site_dropdown', options=[{'label': 'All Sites', 'value': 'ALL'}, {'label': 'CCAFS LC-40', 'value': 'CCAFS LC-40'}, 
...
def get_pie_chart(site_dropdown):
    ...
    if site_dropdown == 'ALL':
        ...
        fig = px.pie(filtered_df, values='class', names='Launch Site', title='Total Success Launches Considering all sites')
    else:
...
@app.callback(
     ...
     [Input(component_id='site_dropdown',component_property='value'),Input(component_id="payload_slider",
...
def Scatter_Chart(site_dropdown,payload_slider):
    if site_dropdown == 'ALL':
        ...
        df  = spacex_df
        filtered = (df['Payload Mass (kg)'] > low) & (df['Payload Mass (kg)'] < high)
        fig = px.scatter(
            df[filtered], x="Payload Mass (kg)", y="class", 
